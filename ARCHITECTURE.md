...
- `StartAttempt` runs immediately before broker I/O and increments `attempts` only while the current `claim_id` still owns the row. Cancellation is checked before acquiring the next singular claim, so work abandoned during shutdown neither holds a fresh lease nor consumes an attempt.
