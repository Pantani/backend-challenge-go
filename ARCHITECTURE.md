...
- Broker publication has `OUTBOX_PUBLISH_TIMEOUT` (10s). Attempt accounting and each durable confirmation, retry, or dead-letter mutation use an independent `OUTBOX_FINALIZE_TIMEOUT` (5s). Both contexts preserve request values while ignoring `SIGTERM`; the configuration requires their overflow-safe sum to be strictly lower than `OUTBOX_LEASE`, without multiplying it by the batch size.
