...
- Migration 5 adds an acquisition-specific `claim_id` to each outbox lease. Its `up` and `down` directions both invalidate active claims, so operations must stop all relays before migrating and restart a binary compatible with the resulting schema afterward.
...
- **Send contract**: `MessageGroupId` is the canonical lowercase, hyphenated `walletId` UUID. This keeps one ordered group per wallet even when the JSON uses uppercase UUID letters. `MessageDeduplicationId` is the envelope's `messageId`. The five-minute FIFO deduplication window is an optimization; durable inbox and idempotency checks provide the guarantee.
...
- The relay (one goroutine per instance) processes rounds of singular acquisitions, acquiring one partition head immediately before publishing it. Each acquisition writes a lease triplet (`locked_by`, `locked_until`, `claim_id`); `claim_id` is a fresh fencing token, so even two relays using the same instance name cannot apply a stale outcome after the lease is reacquired.
...
- `StartAttempt` runs immediately before broker I/O. In one fenced SQL update, it requires the current `claim_id` and an unexpired lease, increments `attempts`, and renews `locked_until` from the real attempt start. An expired claim cannot revive itself, and a reclaimed row rejects the stale token. Cancellation is checked before acquiring the next singular claim, so work abandoned during shutdown neither holds a fresh lease nor consumes an attempt.
- Broker publication has `OUTBOX_PUBLISH_TIMEOUT` (10s). Attempt accounting and its terminal confirmation, retry, or dead-letter mutation share one total `OUTBOX_FINALIZE_TIMEOUT` budget (5s); time spent starting the attempt is deducted before the terminal context is created. Publication and durable contexts preserve request values while ignoring `SIGTERM`. Configuration requires the overflow-safe sum `PublishTime + FinalizeTime` to be strictly lower than `OUTBOX_LEASE`, without multiplying it by the batch size.
- A failed publication releases the lease and schedules `next_attempt_at` with exponential backoff (`attempts`, `last_error`). After `OUTBOX_MAX_ATTEMPTS`, the row receives `dead_lettered_at` and no longer blocks its wallet. Published, failure, and dead-letter metrics are incremented only after the matching fenced mutation succeeds; a lost claim is logged and abandoned.
...
- **Shutdown** (reverse hook registration order): (1) `http.Server.Shutdown` stops admission and drains active requests; (2) the worker group stops, letting the consumer finish its current message or release messages it has not started; (3) the PostgreSQL pool closes **after** its users. `startWorkers` registers the group stop hook after its dependencies, including the pool, and `TestFxStopsServerThenWorkersThenPool` verifies the actual hook order. `SHUTDOWN_TIMEOUT` is `fx.StopTimeout`; its final 5s are reserved for cleanup, and the preceding time must exceed the serial sum of the 30s HTTP write bound and the longest SQS, outbox, or database operation. A second `SIGTERM` or `SIGINT` during shutdown restores Go's default behavior and terminates immediately.
