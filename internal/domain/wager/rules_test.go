...
func TestDecideRollbackOfOpeningIsInvalid(t *testing.T) {
	t.Parallel()
	f := newFixture(t, "100.00")
	opening, err := wager.NewOpening(wager.OpeningParams{ID: uuid.New(), WalletID: f.walletID, PlayerID: f.playerID, Amount: mny(t, "10.00", "BRL"), Now: now})
	require.NoError(t, err)
...
func TestDecideReferenceMismatch(t *testing.T) {
	...
	mutations := map[string]func(p *wager.ExternalParams){
		...
		"currency": func(p *wager.ExternalParams) { p.Amount = mny(t, "10.00", "USD") },
	}
